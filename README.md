...
TSM 是一个基于 Rust 编写的跨平台命令行工具，旨在解决国内开发环境依赖下载速度慢、配置繁琐的问题。它提供"并发测速-对比-自动配置"的一站式解决方案，支持 pip, npm, docker 等多种常见开发工具。

### 🆚 相比 cmirror 的增强

- **首次安装默认配置** — 初次运行自动生成用户配置文件（内置镜像源），后续升级不覆盖，用户配置为唯一数据源
- **交互式工具开关** (`tsm tools`) — 可视化多选启用/禁用工具，禁用工具的命令自动拦截
...
* **🔘 工具开关**: `tsm tools` 交互式管理工具启用/禁用，禁用工具的命令会提示 `not enabled`。
...
```bash
# Linux x64
wget https://github.com/ox01024/tsm/releases/latest/download/tsm-linux-x64.tar.gz
...
```bash
# macOS ARM64
curl -L -o tsm-macos-arm64.tar.gz https://github.com/ox01024/tsm/releases/latest/download/tsm-macos-arm64.tar.gz
...
```bash
git clone https://github.com/ox01024/tsm.git
cd tsm
cargo build --release
